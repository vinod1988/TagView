...
//    [[self appearance] setHoleRadius:kDefaultHoleRadius];
    ...
    [[self appearance] setTagBgColor:kDefaultTagColor];
...
//    CGFloat leftMargin = (int)(self.innerTagPadding + self.tagLength + (self.tagLength ? self.radius / 2 : 0));
    ...
    CGFloat rightMargin = self.innerTagPadding  +10;
...
//        [aPath addArcWithCenter:(CGPoint){width - radius, radius} radius:radius startAngle:DEGREES_TO_RADIANS(0) endAngle:DEGREES_TO_RADIANS(270) clockwise:NO];
        [aPath addLineToPoint:(CGPoint){0, 0.0}];
//        [aPath addArcWithCenter:(CGPoint){tagLength + radius, radius} radius:radius startAngle:DEGREES_TO_RADIANS(270) endAngle:DEGREES_TO_RADIANS(230) clockwise:NO];
        [aPath addLineToPoint:(CGPoint){0, height / 2}];
...
//        [aPath addArcWithCenter:(CGPoint){tagLength / 2 + self.holeRadius, height / 2} radius:self.holeRadius startAngle:DEGREES_TO_RADIANS(180) endAngle:DEGREES_TO_RADIANS(0) clockwise:YES];
        ...
        if (![self.tagBgColor isEqual:self.tagColor]) {
            [self.tagBgColor setFill];
        } else {
    ...
    [background moveToPoint:(CGPoint){left-5 + radius, padding}];
    ...
    [background addLineToPoint:(CGPoint){left + radius, height - padding}];
    [background addArcWithCenter:(CGPoint){ left + radius, height - padding - radius} radius:radius startAngle:DEGREES_TO_RADIANS(90) endAngle:DEGREES_TO_RADIANS(180) clockwise:YES];
    [background addLineToPoint:(CGPoint){left, padding + radius}];
    [background addArcWithCenter:(CGPoint){left + radius, padding + radius} radius:radius startAngle:DEGREES_TO_RADIANS(180) endAngle:DEGREES_TO_RADIANS(270) clockwise:YES];
